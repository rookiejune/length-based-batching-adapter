...
2026-07-23 的 index-only map-style 复旦 smoke 见
[2026-07-23 index-only todo smoke](#2026-07-23-index-only-todo-smoke)。在 145
的 2-GPU Wikitext text-file token-work benchmark 中，quality LBA 仍把 padding ratio
从 `68.16%` 降到 `4.01%`，但 index-only 双阶段读取后 loader wait 明显成为瓶颈：
quality LBA `elapsed=22.83s`，baseline `elapsed=0.45s`。启用
`distributed_cost_window_batches=2` 后，LBA `elapsed` 降到 `11.79s`，step compute
spread 从 `2.39s` 降到 `0.61s`，但这仍是轻量 token-work benchmark，不证明真实模型
吞吐提升。
...
## 2026-07-23 index-only todo smoke

本次复旦测试验证 index-only map-style 数据流合入后，todo 中涉及的 quality planner
padding、candidate window checks、loader wait、token-work DDP step spread 和
distributed cost window metadata 仍可观测。测试机为 `145.pami.group`，使用 GPU
`3,4`，Python `/home/zhuyin/anaconda3/envs/py312/bin/python`，PyTorch
`2.9.0+cu128`，数据为 145 上已有
`~/lba_benchmark_run/lba/outputs/datasets/wikitext103_train_200k.txt` 的前 8,192
条非空文本。命令设置 `NCCL_IB_DISABLE=1 NCCL_P2P_DISABLE=1`，`batch_size=32`、
`num_workers=2`、`planner_mode=quality`、`prefetch_batches=4`、
`compute_iters=4`、`warmup_runs=1`、`repeats=1`。

原始 CSV 保存在远程共享目录：

- `/mnt/pami202/zhuyin/repos/third_party/length-based-batching-adapter/outputs/fdu_todo_20260723/quality.csv`
- `/mnt/pami202/zhuyin/repos/third_party/length-based-batching-adapter/outputs/fdu_todo_20260723/global_k2.csv`

| run | elapsed | loader wait sum | step spread | samples/sec | padding ratio | padded length | candidate checks | max checks | no-ready | remote records |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| baseline | 0.45s | 0.17s | 0.03s | 18,040 | 68.16% | 2,322,304 | 0 | 0 | 0 | n/a |
| quality LBA | 22.83s | 41.03s | 2.39s | 359 | 4.01% | 770,242 | 364,027 | 3,135 | 0 | 0 |
| quality LBA + distributed K=2 | 11.79s | 21.12s | 0.61s | 695 | 4.01% | 770,242 | 364,027 | 3,135 | 0 | 4,382 |

两组 LBA 都保持样本数和 raw length 守恒：`samples=8192`，
`raw_length_sum=739335`。相比 baseline，quality LBA 将 padded length 减少约
66.8%，但 index-only 双阶段读取下 loader wait 明显偏高；这说明当前瓶颈在 producer /
materialization 路径，而不是 planner 无法找到低 padding batch。distributed K=2 在该
轻量 token-work 设置下把 step compute spread 从 `2.39s` 降到 `0.61s`，并把 elapsed
降到 `11.79s`；event log 中共有 256 个 `distributed_cost_block`，累计 remote
batches 232、remote records 4,382，平均 improvement ratio 约 11.3%。该结果说明 global
matching 机制可工作，但仍需真实模型、GPU utilization 和端到端吞吐验证。
