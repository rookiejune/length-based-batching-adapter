...
distributed cost window 的远端 sample 在接收 rank 主进程逐个调用
`dataset[index]`，不会经过 source worker 或 batched `__getitems__`。重 decode、transform
或远端存储读取可能抵消 cost matching 的收益。
...
indexed final flush 和 distributed cost window 都会在接收 rank 主进程重新调用
`dataset[index]`。LBA 只检查 index 是否存在，不判断 lookup 是否稳定。调用侧必须保证
各 rank 的 dataset index-compatible，读取可在 worker 外执行、确定、无副作用，并返回
相同 sample 和有效长度。LBA 会重跑 `len_fn` 并在有效长度变化时显式报错；它无法判断
内容变化但长度相同。改变长度、依赖 worker 或内部 cursor 的随机 transform 不符合该契约。
...
IterableDataset 不支持 `distributed_cost_window_batches`，loader 构造时会直接报错。
