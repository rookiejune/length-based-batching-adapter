...
- `planner_mode="latency"` 使用同样的 recent-window 候选上限，但 capped miss 后不会
  defer 等待更多 source records，而是立刻在当前 planner pool 中运行代表候选 fallback，
  输出当前缓存里最好的 batch。
...
`planner_mode="latency"` 也是显式 opt-in，但目标是减少消费端等待，而不是继续攒样本
追求更低 padding。它复用 throughput 的 recent-window cap；如果 capped threshold search
