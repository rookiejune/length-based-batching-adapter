...
- `planner_mode="latency"`，默认使用和 throughput 相同的 recent-window 候选上限；
  capped miss 后不会 defer 等待更多 source records，而是立即在当前 planner pool 中
  运行代表候选 fallback，输出当前缓存里最好的 batch。
- `planner_mode="quality"` 显式选择不限制候选窗口，使用代表候选 fallback，优先降低
  padding。
...
- throughput 模式默认启用 adaptive 偿还机制：连续 capped search miss 达到
  ...
  全部推迟到 final flush。
- 默认 planner 现在偏向低等待；追求极低 padding 或吞吐实验的策略必须显式 opt-in，
  并在 benchmark 中同时报告 padding、loader wait 和 planner 开销。

## v2 稳定边界

稳定边界是：`LBA` 是直接从 dataset 构造的 DataLoader 子类；`len_fn` 在最终
`collate_fn` 前运行；最终 batch 继续交给用户显式传入的 `collate_fn`；默认 latency
planner 不在 capped miss 后等待未来样本；DDP 下 final flush 保证每个 rank 有相同
步数。

v2 不承诺动态 batch 的精确样本顺序，也不承诺不同 planner 模式之间产生相同 batch
边界。更追求低 padding 的策略需要显式 opt-in，不能静默改变默认 latency 行为。
...
`planner_mode="latency"` 是默认模式，目标是减少消费端等待，而不是继续攒样本追求更低
padding。它复用 throughput 的 recent-window cap；如果 capped threshold search
没有找到达标 batch，会立即在当前 planner pool 里运行代表候选 fallback 并输出 batch。
因此 latency 模式可能接受更高 padding，但不会因为 capped miss 产生 `no_ready` 等待。
...
因此当前默认切换为 latency planner：它保留“当前缓存中选一个代表最优 batch”的 fallback，
但不因为 capped miss 继续等待未来样本。需要继续极致压低 padding 时，调用侧显式选择
