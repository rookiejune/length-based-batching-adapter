...
默认 `planner_mode="latency"` 是低等待基线。它使用有限 recent-window fast path，并在
fast path 没找到达标 batch 时立即从当前 planner pool 运行代表候选 fallback，避免继续
等待未来样本：
...
如果目标是继续极致压低 padding，而不是降低 loader wait，可以显式切回 quality 模式：
...
如果需要手动设置 latency 模式的搜索上限，可以显式传入 `max_candidate_windows`：
...
latency 模式和 throughput 模式一样限制 recent-window search 成本；区别是 capped
search 没找到达标 batch 时不会返回 `None` 等待后续 source records，而是立即在当前
planner pool 中运行代表候选 fallback 并输出当前缓存里最好的 batch。因此它更适合
prefetch queue 被消费端追上、loader wait 比 padding ratio 更重要的训练场景。
